...
import com.cityconnect.widget.MMException;
...
public abstract class BaseHttpHelper extends AsyncTask<Void, Void, BaseResponse>  {
    ...
    public int mApiID;
    private String mApi="";
    protected List<NameValuePair> mJsonInput;
    ...
    protected BaseHttpHelper(String Type,String ApiName,List<NameValuePair> inputParameter,ResponseHelper responseHelper) {
        mApiType = Type;
        mApi=ApiName;
    ...
    public String sendRequest() {
        ...
        try {
            ...
            writer.write(getQuery(mJsonInput));
            writer.flush();
            writer.close();
            os.close();
    ...
    private BaseResponse retriveResponse(String pResponse) throws MMException {
        ...
        switch(mApiID) {
            case ApiConstants.UPLOAD_ISSUE_ID:
                lBaseResponse = BaseResponse.fromJson(pResponse);
                lBaseResponse.setmAPIType(ApiConstants.UPLOAD_ISSUE_ID);
                break;
    ...
    public static boolean isNwConnected(Context context, boolean status) {
        if (context == null) {
            return true;
        }
